...
class DMN_diagram {
    ...
    static _Set_tippy_object(decision, mode) {
        ...
        _a._Get_SVGGElement(decision.id)._tippy.setProps({
            content: mode === undefined
                ? _a._Decision_formats.get(decision)
    ...
    static _Randomize_data(decision, random_size = _a.RandomSize) {
        ...
        const randomization_setup = decision_table.input.every((input_clause, input_clause_index) => {
            ...
            if (type_reference === DMN_type_reference_.BOOLEAN)
            ...
            else if (type_reference === DMN_type_reference_.INTEGER ||
                ...
                if ("inputValues" in input_clause) {
                    ...
                    randomizers.push([
                        Name_of_DMN_InputClause(input_clause),
                        () => Math.floor(Math.random() *
                            (Math.max(...values) + _a._Random_spectrum)),
                    ]);
                }
            }
            else if (type_reference === DMN_type_reference_.DOUBLE ||
                type_reference === DMN_type_reference_.NUMBER) {
                const values = new Set();
                if ("inputValues" in input_clause) {
                    const matches = input_clause
                ...
                decision_table.rule.forEach((rule) => {
                    const matches = rule.inputEntry[input_clause_index].text
                        .replace(/\.\./g, "@")
                        .match(/[-+]?[0-9]*\.?[0-9]+/g);
                    if (matches !== null)
                        matches
                ...
                if ([...values].filter((value) => Number.isInteger(value)).length ===
                    values.size)
                    randomizers.push([
                        Name_of_DMN_InputClause(input_clause),
                        () => Math.floor(Math.random() *
                            (Math.max(...values) + _a._Random_spectrum - min) +
                            min),
                    ]);
                else
                    randomizers.push([
    ...
    static _Decision_data_format(decision) {
        if (Is_DMN_DecisionTable(decision.decisionLogic)) {
            ...
            return ('{<code>"action"</code>: "' +
                ...
                '", <code>"data"</code>: [ {@}, ... ] }'.replace("@", inserter.substring(0, inserter.length - 1)));
        }
            ...
            features.forEach((feature, feature_index) => {
                if (types[feature_index] === DMN_type_reference_.ENUMERATION) {
                    ...
                    if (feature_index < features.length - units)
                        // Input...
                        value =
                            decision_maker.enumerations.get(feature)[datum[feature_index].indexOf(1)];
                    else {
                        // Output...
                    ...
                    Object.defineProperty(data_[datum_index], feature, {
                        value: types[feature_index] === DMN_type_reference_.INTEGER ||
                            ...
                            ? Math.round(datum[feature_index])
                            : datum[feature_index],
                        enumerable: true,
                        configurable: false,
                        writable: true,
        ...
        data_.forEach((datum, datum_index) => {
            ...
            keys.forEach((key, key_index) => {
                if (mode !== Drop_mode.PREDICT || key_index < features.length - units) {
                    ...
                    if (decision_maker.types[feature_index] ===
                        ...
                        data_[datum_index][feature_index] =
                            one_hot_encoding;
                    }
                    else
                        data_[datum_index][feature_index] =
    ...
    static _Download(status, data, decision) {
        ...
        download.setAttribute("download", (decision.name ? decision.name : _a._File_name) +
            "_" +
            status +
            ".json");
        download.click();
    ...
    async view() {
        ...
        if (diagram.drgElement
            .filter((me) => Is_DMN_Decision(me) && Is_DMN_DecisionTable(me.decisionLogic))
            .every((decision) => {
            try {
                ...
                _a._Display_error(error instanceof DMiNer_error
                    ? error
                    : new DMiNer_error(_a._Decision, DMiNer_error.Invalid_JSON), 5000);
                            ...
                            train_button.onclick = () => {
                                ...
                                setTimeout(() => {
                                    ...
                                    _a._Randomize_data(decision)
                                        ...
                                        setTimeout(() => {
                                            // Long-running processing...
                                            _a.Decisions.get(decision)
                                                .train(_a._To_TensorFlow_data(Drop_mode.TRAIN, decision, data))
                                                .then((message) => {
    ...
    static _Setup_decision_maker(diagram, decision) {
        ...
        decision.informationRequirement?.forEach((me) => {
            if (me.requiredDecision) {
                const required_decision = diagram.drgElement
                    .filter(Is_DMN_Decision)
                    .filter((decision_) => "#" + decision_.id === me.requiredDecision.href)
                    .pop();
            ...
            if (me.requiredInput) {
                const required_input = diagram.drgElement
                    .filter(Is_DMN_InputData)
                    .filter((input_data) => "#" + input_data.id === me.requiredInput.href)
                    .pop();
         ...
         */
        const enumerations_ = new Map(decision_table.input.map((input_clause) => [
            Name_of_DMN_InputClause(input_clause),
        ...
        const enumerations = new Map([
            ...enumerations_,
            ...new Map(decision_table.output.map((output_clause) => [
                        ...
                        if (_a._Is_idle_(decision)) {
                            ...
                            input_TensorFlow_data.forEach((datum, datum_index) => {
                                decision_maker.features
                                    .filter((feature, feature_index) => feature_index >=
                                    decision_maker.features.length - decision_maker.units) // Outputs...
                                    .forEach((feature) => {
