...
export default class DMN_diagram {
  ...
  private static readonly _Listeners: Map<
    string,
  ...
  private static _Record_listener(
    type: string,
  ...
  private static _Display(message: string, duration = 5000): void {
    const dialog: HTMLDialogElement = window.document.getElementById(
      "Dialog"
  ...
  private static _Get_SVGGElement(id?: string): SVGGElement {
    ...
    return <SVGGElement>(
      window.document.querySelector("#LiveDMN [data-element-id=" + id + "]")
    ); // See "LiveDMN" as id. in 'LiveDMN.com.html'...
  }

  private static readonly _Decision_formats: Map<DMN_Decision, string> =
    new Map();
  ...
  private static _Is_idle_(decision: DMN_Decision): boolean {
    ...
    return (Object.values(Drop_mode) as string[])
      .concat(Object.values(State_mode) as string[])
      .reduce(
        (idle, value) => idle && !sVG_element.classList.contains(value),
        true
      );
  }

  private static _Set_CSS_class(
    decision: DMN_Decision,
    mode?: Drop_mode | State_mode
  ): void {
    const sVG_element = DMN_diagram._Get_SVGGElement(decision.id);
    (Object.values(Drop_mode) as string[])
      .concat(Object.values(State_mode) as string[])
      .forEach((mode) => sVG_element.classList.remove(mode));
    if (mode) sVG_element.classList.add(mode);
  }

  private static _Set_tippy_object(
    decision: DMN_Decision,
    mode?: Drop_mode | State_mode
  ): void {
    // @ts-ignore
    DMN_diagram._Get_SVGGElement(decision.id)._tippy.setProps({
      content:
        mode === undefined
          ? DMN_diagram._Decision_formats.get(decision)
          : mode === State_mode.MENU
          ? DMN_diagram._Menu
          : mode,
  ...
  private static _Randomize_data(
    decision: DMN_Decision,
    random_size: number = DMN_diagram.RandomSize
  ): Promise<Data["data"]> {
    if (!Is_DMN_DecisionTable(decision.decisionLogic))
      return Promise.reject(
        new DMiNer_error(decision, DMiNer_error.No_business_logic)
      );
    let data: Data["data"] = new Array();
    const decision_table = decision.decisionLogic;
    const randomizers: Array<[string, Function]> = new Array();
    const randomization_setup = decision_table.input!.every(
      (input_clause, input_clause_index) => {
        const type_reference: DMN_type_reference_ = Type_of_DMN_InputClause(
          input_clause,
          decision
        );
        if (type_reference === DMN_type_reference_.BOOLEAN)
          ...
          if ("inputValues" in input_clause) {
            const matches: null | RegExpMatchArray =
              input_clause.inputValues!.text.match(/\d+/g);
            if (matches !== null)
              matches
                .map((match) => Number.parseInt(match))
            ...
            decision_table.rule!.forEach((rule) => {
              const matches: null | RegExpMatchArray =
                rule.inputEntry[input_clause_index].text.match(/\d+/g);
              if (matches !== null)
                matches
                  .map((match) => Number.parseInt(match))
                  .forEach(values.add, values);
            ...
            randomizers.push([
              ...
              () =>
                Math.floor(
                  Math.random() *
                    (Math.max(...values) + DMN_diagram._Random_spectrum)
                ),
            ]);
          }
        } else if (
          ...
          const values: Set<number> = new Set();
          if ("inputValues" in input_clause) {
            const matches: null | RegExpMatchArray = input_clause
              .inputValues!.text.replace(/\.\./g, "@")
              .match(/[-+]?[0-9]*\.?[0-9]+/g);
            if (matches !== null)
              matches
          ...
          decision_table.rule!.forEach((rule) => {
            const matches: null | RegExpMatchArray = rule.inputEntry[
              input_clause_index
            ].text
              .replace(/\.\./g, "@")
              .match(/[-+]?[0-9]*\.?[0-9]+/g);
          ...
          if (
            ...
            randomizers.push([
              ...
              () =>
                Math.floor(
                  Math.random() *
                    (Math.max(...values) + DMN_diagram._Random_spectrum - min) +
                    min
                ),
            ]);
          else
            randomizers.push([
              ...
              () =>
                Math.random() *
                  (Math.max(...values) +
                    DMN_diagram._Random_spectrum / 10 -
                    min) +
                min,
            ]);
          ...
          decision_table.rule!.forEach((rule) => {
            const ast: Array<Object> | undefined =
              IMICROS_FEEL_interpreter.Parse(
                rule.inputEntry[input_clause_index]
              );
          });
          return false; // 'every'
        } else {
          ...
          const values = DMN_diagram.Decisions.get(decision)!.enumerations.get(
            Name_of_DMN_InputClause(input_clause)
          );
          randomizers.push([
            Name_of_DMN_InputClause(input_clause),
    ...
    if (!randomization_setup)
      return Promise.reject(
        new DMiNer_error(decision, DMiNer_error.No_possible_randomization)
      );
    ...
    return new Promise<Data["data"]>(async (send) => {
      ...
      data = data.filter((datum) => {
        ...
        if (features.length !== keys.length - 1) return false;
        return Object.keys(datum).every(
          (key) => key.includes(_DMiNer_) || features.includes(key)
        );
      });
      send(data);
    });
  ...
  private static _Decision_data_format(decision: DMN_Decision): string {
    if (Is_DMN_DecisionTable(decision.decisionLogic)) {
      ...
      return (
        '{<code>"action"</code>: "' +
        Object.values(Drop_mode).join('" | "') +
        '", <code>"data"</code>: [ {@}, ... ] }'.replace(
          "@",
          inserter.substring(0, inserter.length - 1)
        )
      );
  ...
  private static _From_TensorFlow_data(
    decision: DMN_Decision,
    data: TensorFlow_data
  ): Data["data"] {
    ...
    data.forEach((datum, datum_index) => {
      ...
      features.forEach((feature, feature_index) => {
        if (types[feature_index] === DMN_type_reference_.ENUMERATION) {
          if (Trace)
            console.assert(
              Array.isArray(datum[feature_index]),
              "DMN_diagram._From_TensorFlow_data >> 'Array.isArray(datum[feature_index])', untrue"
            );
          let value: any;
          if (feature_index < features.length - units)
            // Input...
            value =
              decision_maker!.enumerations.get(feature)![
                (datum[feature_index] as Array<0 | 1>).indexOf(1)
              ];
          else {
            // Output...
            const probability = Math.max(
              ...(datum[feature_index] as Array<0 | 1>)
            );
            const index = (datum[feature_index] as Array<0 | 1>).indexOf(
              probability as 0 | 1
            );
          ...
          Object.defineProperty(data_[datum_index], feature, {
            value:
              types[feature_index] === DMN_type_reference_.INTEGER ||
              types[feature_index] === DMN_type_reference_.LONG
                ? Math.round(datum[feature_index] as number)
                : datum[feature_index],
            enumerable: true,
            configurable: false,
            writable: true,
  ...
  private static _To_TensorFlow_data(
    mode: Drop_mode,
    decision: DMN_Decision,
    ...
    data.forEach((datum) =>
      Object.keys(datum).forEach((key) => {
        if (key.includes(_DMiNer_)) delete datum[key as keyof typeof datum];
      })
    );
    ...
    data_ = data_.filter((datum) =>
      Object.values(datum).every((value, value_index) => {
        ...
        type_reference =
          ...
          type_reference === DMN_type_reference_.INTEGER ||
          type_reference === DMN_type_reference_.LONG
            ? DMN_type_reference_.NUMBER
            : type_reference;
        return type_reference === DMN_type_reference_.ENUMERATION
          ? typeof value === DMN_type_reference_.BOOLEAN ||
              typeof value === DMN_type_reference_.STRING ||
              (typeof value === DMN_type_reference_.NUMBER &&
    ...
    data_.forEach((datum, datum_index) => {
      ...
      keys.forEach((key, key_index) => {
        if (mode !== Drop_mode.PREDICT || key_index < features.length - units) {
          ...
          if (
            decision_maker!.types[feature_index] ===
            DMN_type_reference_.ENUMERATION
          ) {
            ...
            const one_hot_encoding = decision_maker!.enumerations
              .get(key)!
              .map((value) => (value === values[key_index] ? 1 : 0));
            (data_[datum_index] as TensorFlow_datum)[feature_index] =
              one_hot_encoding;
          } else
            (data_[datum_index] as TensorFlow_datum)[feature_index] =
  ...
  private static _Download(
    ...
    const download: HTMLAnchorElement = window.document.getElementById(
      "Download"
    ...
    download.setAttribute(
      "download",
      (decision.name ? decision.name : DMN_diagram._File_name) +
        "_" +
        status +
        ".json"
    );
  ...
  public static readonly Decisions: Map<DMN_Decision, Decision_maker> =
    new Map();
  ...
  static {
    ...
    DMN_diagram.File_reader.onload = async (
      progress_event: ProgressEvent<FileReader>
    ): Promise<never | void> => {
      // if (Trace)
      ...
      try {
        ...
        if (Is_Data(data!)) {
          ...
          const decision_maker: Decision_maker =
            DMN_diagram.Decisions.get(decision)!;
          switch (data.action) {
            case Drop_mode.FEEL:
            ...
            case Drop_mode.PREDICT:
              ...
              if (DMN_diagram._Is_idle_(decision)) {
                ...
                const input_TensorFlow_data = DMN_diagram._To_TensorFlow_data(
                  Drop_mode.PREDICT,
                  decision,
                  data.data
                );
                const output_TensorFlow_data: TensorFlow_datum =
                  await decision_maker.predict(input_TensorFlow_data);
                // if (Trace)
                //     console.assert(input_TensorFlow_data.length === output_TensorFlow_data.length);
                input_TensorFlow_data.forEach((datum, datum_index) => {
                  decision_maker.features
                    .filter(
                      (feature, feature_index) =>
                        feature_index >=
                        decision_maker.features.length - decision_maker.units
                    ) // Outputs...
                    .forEach((feature) => {
                      datum.push(
                        (
                          output_TensorFlow_data[datum_index] as Array<0 | 1>
                        ).splice(
                          0,
                          decision_maker.get_enumeration_dimension(feature)
                        )
                      );
                    });
                });
                DMN_diagram._Download(
                  Status_mode.PREDICTED,
                  DMN_diagram._From_TensorFlow_data(
                    decision,
                    input_TensorFlow_data
                  ),
                  decision
                );
              }
            ...
            case Drop_mode.TRAIN:
              if (DMN_diagram._Is_idle_(decision)) {
                ...
                await decision_maker.train(
                  DMN_diagram._To_TensorFlow_data(
                    Drop_mode.TRAIN,
                    decision,
                    data.data
                  )
            ...
            case Drop_mode.VISUALIZE:
              ...
              break;
            default:
              throw new DMiNer_error(
                decision,
                DMiNer_error.Invalid_drop_mode,
                data.action
              );
          }
        } else {
          throw new DMiNer_error(
            DMN_diagram._Decision!,
            DMiNer_error.Invalid_data_format
          );
  ...
  private async _get(): Promise<DMN_Definitions | undefined> {
    ...
    try {
      ...
      if (Trace)
        console.assert(
          Is_DMN_Definitions(diagram),
          "'DMN_diagram._get_diagram' >> 'Is_DMN_Definitions(diagram)', untrue."
        );
      if (Trace && warnings.length !== 0)
        console.warn(
          _DMiNer_ + warnings.map((warning: any) => warning.message).join(" * ")
        );
      return Promise.resolve(diagram);
    } catch (error: unknown) {
      DMN_diagram._Display(
        "'DMN_diagram._get_diagram' >> XML in DMN file cannot be processed."
      );
  ...
  async view(): Promise<undefined | void> {
    const diagram = await this._diagram;
    if (!diagram) return Promise.resolve(undefined); // Content is not DMN...
    ...
    if (Trace && warnings.length !== 0)
      console.warn(
        "DMiner: " + warnings.map((warning: any) => warning.message).join(" * ")
      );
    ...
    if (
      diagram.drgElement
        .filter(
          (me) => Is_DMN_Decision(me) && Is_DMN_DecisionTable(me.decisionLogic)
        )
        .every((decision) => {
          try {
            DMN_diagram._Setup_decision_maker(
              diagram,
              decision as DMN_Decision
            );
            return true; // Because of 'every'...
          } catch (error: unknown) {
            DMN_diagram._Display_error(
              error instanceof DMiNer_error
                ? error
                : new DMiNer_error(
                    DMN_diagram._Decision!,
                    DMiNer_error.Invalid_JSON
                  ),
              5000
            );
    ...
    diagram.drgElement
      ...
      .forEach((decision: DMN_Decision) => {
        ...
        tippy(sVG_element, {
          ...
          onTrigger(tippy_object: any, event: MouseEvent) {
            // console.info('onTrigger ' + event.type);
            if (event.type === "click" && DMN_diagram._Is_idle_(decision)) {
              ...
              setTimeout(() => {
                // Wait for 'tippy.js' to mount menu on DOM...
                const randomize_button: HTMLButtonElement =
                ...
                const train_button: HTMLButtonElement =
                  window.document.getElementById(
                    "TRAIN_BUTTON"
                  ) as HTMLButtonElement;
                // if (Trace)
                ...
                const dataviz_button: HTMLButtonElement =
                  window.document.getElementById(
                    "DATAVIZ_BUTTON"
                  ) as HTMLButtonElement;
                // if (Trace)
        ...
        sVG_element.addEventListener(
          ...
          (event: DragEvent) => {
            ...
            if ("dataTransfer" in event) {
              let i = 0;
              while (event.dataTransfer!.items[i].kind !== "file") i++;
              // window.alert(event.dataTransfer!.items[i].getAsFile()!.name);
              // Load data:
              DMN_diagram.File_reader.readAsText(
              ...
              DMN_diagram._File_name =
                event.dataTransfer!.items[i].getAsFile()!.name;
            } else
              throw new Error(
                "'sVG_element.addEventListener('drop', ...' >> ''dataTransfer' in event', untrue."
              );
          },
          false
        );
        DMN_diagram._Decision_formats.set(
          decision,
          DMN_diagram._Decision_data_format(decision)
        );
      });
    /**
  ...
  private static _Setup_decision_maker(
    diagram: DMN_Definitions,
    decision: DMN_Decision
  ): never | void {
    DMN_diagram._Clear_listeners(Name_of_DMN_Decision(decision));
    DMN_diagram._Record_listener(
      Decision_maker.onTrainBegin + Name_of_DMN_Decision(decision),
      (event) => {
        console.info(
          Drop_mode.TRAIN + " begins... " + Name_of_DMN_Decision(decision)
        );
      }
    );
    DMN_diagram._Record_listener(
      Decision_maker.onTrainEnd + Name_of_DMN_Decision(decision),
      (event) => {
        console.info(
          Drop_mode.TRAIN +
            " ends... " +
            Name_of_DMN_Decision(decision) +
            ", loss: " +
            (
    ...
    decision.informationRequirement?.forEach(
      (me: DMN_InformationRequirement) => {
        if (me.requiredDecision) {
          const required_decision: DMN_Decision | undefined = diagram.drgElement
            .filter(Is_DMN_Decision)
            .filter(
              (decision_) => "#" + decision_.id === me.requiredDecision!.href
            )
            .pop();
          if (required_decision === undefined)
            throw new DMiNer_error(
              me.requiredDecision,
              DMiNer_error.Inconsistent_DMN_diagram,
              "not found..."
            );
          required_decisions.push(required_decision);
        }
        if (me.requiredInput) {
          const required_input: DMN_InputData | undefined = diagram.drgElement
            .filter(Is_DMN_InputData)
            .filter(
              (input_data) => "#" + input_data.id === me.requiredInput!.href
            )
            .pop();
     ...
     */
    const enumerations_: Map<string, Array<boolean | number | string> | null> =
      new Map(
        decision_table.input!.map((input_clause) => [
          Name_of_DMN_InputClause(input_clause),
          Type_of_DMN_InputClause(input_clause, decision) ===
          DMN_type_reference_.ENUMERATION
            ? Get_enumeration_from_DMN_InputClause(input_clause)
            : null,
        ])
      );
    let features = decision_table.input!.map((input_clause) =>
      Name_of_DMN_InputClause(input_clause)
    );
    let types = decision_table.input!.map((input_clause) =>
      Type_of_DMN_InputClause(input_clause, decision)
    );
    ...
    // }));
    const enumerations: Map<string, Array<boolean | number | string> | null> =
      new Map([
        ...enumerations_,
        ...new Map(
          decision_table.output!.map((output_clause) => [
            Name_of_DMN_OutputClause(output_clause),
            Type_of_DMN_OutputClause(output_clause, decision) ===
            DMN_type_reference_.ENUMERATION
