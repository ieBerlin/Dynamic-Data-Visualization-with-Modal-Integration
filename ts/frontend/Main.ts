...
class DMiNer {
  ...
  static readonly Enable_load_file = (): void => {
    ...
    DMiNer.File_reader.onload = (progress_event: ProgressEvent<FileReader>) => {
      if (Trace)
        console.assert(
          progress_event.target === DMiNer.File_reader,
          "'DMiNer.File_reader.onload' >> 'progress_event.target === DMiNer.File_reader', untrue."
        );
      if (Trace)
        console.assert(
          "file_name" in window.history.state,
          "'DMiNer.File_reader.onload' >> '\"file_name\" in window.history.state', untrue."
        );

      DMiNer.Test_case(
        window.history.state.file_name,
        DMiNer.File_reader.result as string
      );
  ...
  static Change_screen(screen: string) {
    ...
    const screens: HTMLCollection =
      window.document.getElementsByClassName("Screen");
...
window.addEventListener("DOMContentLoaded", async (_) => {
  DMiNer.Dataviz_area = window.document.getElementById(
    "Dataviz_area"
  ) as HTMLDivElement;
  ...
  const collection: HTMLCollection =
    window.document.getElementsByClassName("TC");
  ...
  collection.item(1)!.addEventListener("click", (event) => {
    DMiNer.Test_case(
      "./TEST_CASES/Get_barcode_country/Get_barcode_country.dmn"
    );
    window.document.body.dispatchEvent(new Event(DMiNer.Reload));
  });
  collection.item(2)!.addEventListener("click", (event) => {
    DMiNer.Test_case(
      "./TEST_CASES/Trisotech/Car_Damage_Responsibility/Car Damage Responsibility.dmn"
    );
    window.document.body.dispatchEvent(new Event(DMiNer.Reload));
  });
  collection.item(3)!.addEventListener("click", (event) => {
    DMiNer.Test_case(
      "./TEST_CASES/Trisotech/Determine_the_Repair_Location/Determine the Repair Location.dmn"
    );
    window.document.body.dispatchEvent(new Event(DMiNer.Reload));
  });
  ...
  window.document
    ...
    .addEventListener(
      "mouseenter",
      (event) => DMiNer.Change_screen("Information_"),
      false
    );
  window.document
    .getElementById("Information")!
    .addEventListener(
      "mouseleave",
      (event) => DMiNer.Change_screen("LiveDMN"),
      false
    );
  window.document
    .getElementById("Bug")!
    .addEventListener(
      "mouseenter",
      (event) => DMiNer.Change_screen("Bug_"),
      false
    );
  window.document
    .getElementById("Bug")!
  ...
  window.document
    ...
    .addEventListener(
      "mouseenter",
      (event) => DMiNer.Change_screen("Dataviz_area"),
      false
    );
  window.document
    .getElementById("Dataviz_icon")!
